...
class Habits:
    ...
    def __init__(self, name: str, timespan: str):
        """Initalizes a new habit. An active habit must have an unique name.
        Timespans have to be defined in the database, by default 'week' and
        'day' are implemented. Checks, if entered name is already used for
        an active habit and, if the entered timespan is present in the database."""

        # Checks if entered name is already in the database.
        habit_names = get_habit_names(get_db(), 0)
        li_habit_names = []
        for all in habit_names:
            li_habit_names.append(all[0])
    ...
    def deactivate_habit(name):
        ...
        if type(habit_id) == str:
            return print(habit_id)
...
class Completion:
    ...
    def check_habit(self, name: str):
        ...
        if type(self.habit_id) == str:
            return print(self.habit_id)
    ...
    def __check_in_time(self):
        ...
        self.timespan_name, self.timespan_days = get_habits_timespan(
            get_db(), self.habit_id)
        ...
        if len(self.li_checks) == 0:
            ...
            date_time_last_check = datetime.strptime(
                self.li_checks[-1][0], '%Y-%m-%d %H:%M:%S')
    ...
    def __streak(self):
        ...
        if self.li_checks[-1][1] == 1:
            ...
            streak_msg = "You have successfully established a streak of " + \
                str(periods) + " periods! Super!"
