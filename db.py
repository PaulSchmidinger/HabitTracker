...
def add_habit(db, name:str, timespan:int, active:int = 1):
    ...
    cur = db.cursor()
    cur.execute("INSERT INTO Habits (name, active, timespan) VALUES (?, ?, ?)", (name, active, timespan))
    db.commit()

def add_check(db, habit_id:int, in_time:int):
    """Stores new check in database."""
    cur = db.cursor()
    cur.execute("INSERT INTO Checks (habit_id, in_time) VALUES (?, ?)", (habit_id, in_time))
    db.commit()
...
def get_habits(db, all:int=1, timespan:str="all"):
    ...
    if timespan == "all":
        if all == 1:
            cur.execute("SELECT Habits.name, Habits.create_d, Timespans.timespan, Habits.active FROM Timespans JOIN Habits ON Timespans.id = Habits.timespan")
        else:
            cur.execute("SELECT Habits.name, Habits.create_d, Timespans.timespan FROM Timespans JOIN Habits ON Timespans.id = Habits.timespan WHERE Habits.active=1")
    else:
        if all == 1:
...
def get_streak_data(db, timespan:str = "all", habitname:str = "all"):
    ...
    df_streak_data = pd.DataFrame(cur.fetchall(),columns = ["Habitname","Time_checked","in_time"])
