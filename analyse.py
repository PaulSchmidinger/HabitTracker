...
def return_habits(all: int = 1, timespan: str = "all"):
    """Returns a list of all or all active habits. When a periodicity is specified, it will only return the habits with this periodicity."""
    li_habits = get_habits(get_db(), all=all, timespan=timespan)

    if len(li_habits) == 0:
        print("There are no such habits!")
...
def return_longest_streak(habitname: str = "all", timespan: str = "all"):
    """Returns the longest run streak of all defined habits with the same periodicity or of a given habit. You have to specify either or."""
    df_streak_data = get_streak_data(
        get_db(), habitname=habitname, timespan=timespan)
    ...
    if timespan == "all" and habitname != "all":
        ...
        if max(li_maxl) == 0:
            print("Your longest run streak for '"+habitname +
                  "' is: "+str(max(li_maxl))+" Periods.")
        else:
            print("Your longest run streak for '"+habitname+"' is: " +
                  str(max(li_maxl))+" Periods. Congratulations!!")
    ...
    # by timespan
        ...
        if max(li_maxl) == 0:
            print("Your longest run streak for the timespan '" +
                  timespan+"' is: "+str(max(li_maxl))+" Periods.")
        else:
